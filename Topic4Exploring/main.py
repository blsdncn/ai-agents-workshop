...
def ddg(query: str) -> str:
    ...
    Args:
        query: String to input into search

    Returns:
        Result of the DDG query.
    """
    try:
        if random.randint(1, 10) == 1:
            return ddg_search.invoke("What's Obama's last name?")
        else:
...
def wikipedia(query: str) -> str:
    ...
    Returns:
        Result of the wikipedia query.
    """
    try:
        if random.randint(1, 10) == 1:
            return wiki.run("What's Obama's last name?")
        else:
...
def call_react_agent(state: ConversationState) -> ConversationState:
    ...
    if state.get("verbose", True):
        ...
        print("=" * 80)
        print(
            f"[DEBUG] Invoking ReAct agent with {len(state['messages'])} messages in history"
        )
        ...
        for msg in result["messages"][messages_before:]:
            if isinstance(msg, AIMessage):
                if hasattr(msg, "tool_calls") and msg.tool_calls:
                    print(
                        f"[DEBUG] Tool calls: {[tc['name'] for tc in msg.tool_calls]}"
                    )
                elif msg.content:
...
def trim_history(state: ConversationState) -> ConversationState:
    ...
    if len(messages) > max_messages:
        ...
        if messages and isinstance(messages[0], SystemMessage):
            ...
            if state.get("verbose", True):
                print(
                    f"[DEBUG] Preserved system message + {max_messages - 1} recent messages"
                )
        else:
...
) -> Literal["call_react_agent", "end", "input"]:
    """
...
def create_conversation_graph():
    ...
    react_agent = create_react_agent(model=model, tools=tools, prompt=system_message)
